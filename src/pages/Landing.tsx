  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none z-0">
      {particles.map((p) => (
        <motion.div
          key={p.id}
          className="absolute bg-rose-500 rounded-full blur-[1px]"
          style={{
            width: p.size,
            height: p.size,
            left: '50%',
...
const HUDCircle = ({ size, duration, reverse = false, dashed = false }: { size: string, duration: number, reverse?: boolean, dashed?: boolean }) => (
  <motion.div
    className={`absolute rounded-full border border-rose-500/20 ${dashed ? 'border-dashed' : ''}`}
...
export default function Landing() {
  ...
  return (
    <div className="min-h-screen bg-stone-950 text-stone-300 font-sans selection:bg-rose-500/30 selection:text-rose-200 relative overflow-hidden flex flex-col items-center justify-center">
      {/* BACKGROUND ELEMENTS */}
      ...
      {/* CENTRAL HUD UNIT */}
      <div className="relative z-10 flex items-center justify-center w-full max-w-4xl h-[600px]">
        {/* Layered HUD Rings */}
        ...
        <motion.div 
          className="absolute w-[600px] h-[1px] bg-gradient-to-r from-transparent via-rose-500/40 to-transparent z-10"
          animate={{ rotate: 360 }}
        ...
        >
          ...
          >
            <div className="relative">
              ...
              <Database className="w-16 h-16 text-rose-500 relative z-10 drop-shadow-[0_0_15px_rgba(225,29,72,0.8)]" />
            </div>
          ...
          <div className="space-y-2">
            <motion.h1 
              ...
              className="text-6xl md:text-8xl font-black text-white uppercase tracking-[0.2em] relative"
            >
              Living<br/>History
              <span className="absolute -inset-1 bg-rose-500/10 blur-2xl -z-10"></span>
            </motion.h1>
            <motion.p 
            ...
            >
              Universal Chronology Archive // v2.05
            </motion.p>
          </div>
          ...
          >
            <Link to="/archive" className="group relative px-10 py-4 block">
              <div className="absolute inset-0 bg-rose-500/50 blur opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
              <div className="relative px-8 py-3 bg-stone-950 border border-rose-500/50 flex items-center gap-4 text-rose-400 font-mono tracking-widest text-sm uppercase group-hover:bg-rose-600 transition-all group-hover:text-white group-hover:border-rose-300 overflow-hidden">
